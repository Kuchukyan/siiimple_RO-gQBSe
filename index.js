...
let onUpdate = function(evt) {
        evt.target.object3D.rotation.y += RODIN.Time.deltaTime() / 500;
};
let onHover = function(evt) {
    evt.target.object3D.scale.set(2, 2, 2);
};
let onHoverOut = function(evt) {
    evt.target.object3D.scale.set(1, 1, 1);
};

for (let i = 0; i < 1000; i++) {
    let cube = new RODIN.THREEObject(new THREE.Mesh(geometry, material));
    cube.on(RODIN.CONSTANTS.EVENT_NAMES.READY, onReady);
    cube.on(RODIN.CONSTANTS.EVENT_NAMES.UPDATE, onUpdate);
    cube.on(RODIN.CONSTANTS.EVENT_NAMES.CONTROLLER_HOVER, onHover);
    cube.on(RODIN.CONSTANTS.EVENT_NAMES.CONTROLLER_HOVER_OUT, onHoverOut);
}
